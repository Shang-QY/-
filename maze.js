var ctxs, wid, hei, cols, rows, mazes, endflag = true, stacks = [], start = [{x:-1, y:-1}, {x:-1, y:-1}], end = [{x:-1, y:-1}, {x:-1, y:-1}],grid = 8, padding = 16, s, density=0.5, count=2;
...
function getFNeighbours2( index, sx, sy, a ) {
    ...
    for(var dx = -1; dx <= 1; ++dx){
        for(var dy = -1; dy <= 1; ++dy){
            if( sx + dx >= 0 && sx + dx < cols && sy + dy >= 0 && sy + dy < rows && mazes[index][sx + dx][sy + dy] % 8 == a ) {
                n.push( { x:sx + dx, y:sy + dy } );
            }
        }
    }
    return n;
...
function getFNeighbours2New( index, sx, sy, a ) {
    var dir, x, y, n = [], v = [];
    if(sx < end[index].x) {
        if(sy < end[index].y) dir = 1;
        else if(sy == end[index].y) dir = 2;
        else dir = 3;
    }
    else if(sx == end[index].x) {
        if(sy < end[index].y) dir = 4;
        else dir = 5;
    }
    else{
        if(sy < end[index].y) dir = 6;
        else if(sy == end[index].y) dir = 7;
        else dir = 8;
    }

    n[0] = [{x:1,y:1},{x:0,y:1},{x:1,y:0},{x:-1,y:1},{x:1,y:-1},{x:-1,y:0},{x:0,y:-1},{x:-1,y:-1}];
    n[1] = [{x:1,y:0},{x:1,y:1},{x:1,y:-1},{x:0,y:1},{x:0,y:-1},{x:-1,y:1},{x:-1,y:0},{x:-1,y:-1}];
    n[2] = [{x:1,y:-1},{x:1,y:0},{x:0,y:-1},{x:-1,y:-1},{x:1,y:1},{x:-1,y:0},{x:0,y:1},{x:-1,y:1}];
    n[3] = [{x:0,y:1},{x:1,y:1},{x:-1,y:1},{x:-1,y:0},{x:1,y:0},{x:-1,y:-1},{x:1,y:-1},{x:0,y:-1}];
    n[4] = [{x:0,y:-1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:0},{x:1,y:0},{x:-1,y:1},{x:1,y:1},{x:0,y:1}];
    n[5] = [{x:-1,y:1},{x:0,y:1},{x:-1,y:0},{x:-1,y:-1},{x:1,y:1},{x:0,y:-1},{x:1,y:0},{x:1,y:-1}];
    n[6] = [{x:-1,y:0},{x:-1,y:-1},{x:-1,y:1},{x:0,y:1},{x:0,y:-1},{x:1,y:-1},{x:1,y:1},{x:1,y:0}];
    n[7] = [{x:-1,y:-1},{x:0,y:-1},{x:-1,y:0},{x:1,y:-1},{x:-1,y:1},{x:1,y:0},{x:0,y:1},{x:1,y:1}];
    
    for(var i = 0; i < 8; ++i){
        x = n[dir-1][i].x;
        y = n[dir-1][i].y;
        if( sx + x >= 0 && sx + x < cols && sy + y >= 0 && sy + y < rows && mazes[index][sx + x][sy + y] % 8 == a ) {
            v.push( { x:sx + x, y:sy + y } );
        }
    }
...
function solveMaze2New(index) {
    ...
    var neighbours = getFNeighbours2New( index, start[index].x, start[index].y, 0 );
    if( neighbours.length ) {
        stacks[index].push( start[index] );
        start[index] = neighbours[0];
        mazes[index][start[index].x][start[index].y] = 2;
...
function getFNeighbours1New(index, sx, sy, a) {
...
function solveMaze1New(index) {
    ...
    var neighbours = getFNeighbours1New( 1, start[index].x, start[index].y, 0 );
    if( neighbours.length ) {
        stacks[index].push( start[index] );
        start[index] = neighbours[0];
        mazes[index][start[index].x][start[index].y] = 2;
...
function getCursorPos( event ) {
    ...
    if( !endflag ) return;//1.不结束不让点 2.点击导致强行结束
    if(end[0].x != -1) {//这句话很精妙，这里顺序经过调整
        onClear();
    }
    //当右边先结束时直接点击会出现问题+经分析是solveMaze未结束，解决方式见1、2
    if( start[0].x == -1 ) {
        start[0] = { x: x, y: y };
        start[1] = { x: x, y: y };
...
function createMaze1() {
    ...
    if( neighbours.length < 1 ) {
        ...
        var i = 2 * Math.floor( Math.random() * ( neighbours.length / 2 ) )//这是很神奇的一句话
        l = neighbours[i]; 
        mazes[0][l.x][l.y] = 0;
        mazes[1][l.x][l.y] = 0;
